...
*  2-May-06 | REG #017: Use REG_QWORD for 64-bit values
*  2-May-06 | REG #018: Support Delete() function on RegistryVar type
*  2-May-06 | REG #019: Support for /Wp64 added, eliminate warnings
*  5-May-06 | REG #020: Handle string values that are NUL-terminated
*****************************************************************************/
...
*                             DeleteRegistryValue
* Inputs:
*       HKEY root: Root key of path
*       const CString & path: Path to value, not including value
*       const CString & name: Name of value to delete
* Result: BOOL
*       TRUE if successful
*       FALSE if error
* Effect: 
...
BOOL DeleteRegistryValue(HKEY root, const CString & var)
    {
     ...
     return DeleteRegistryValue(root, path, name);                  // REG #018
    }

/****************************************************************************
*                              DeleteRegistryKey
...
*       FALSE in unsuccessful, value is unchanged
* Errors:
*       ERROR_INVALID_DATATYPE: Value is not REG_BINARY or REG_QWORD
...
BOOL GetRegistryInt64(HKEY root, const CString & var, __int64 & val) // REG #013
   {
    ...
    if(type != REG_BINARY && type != REG_QWORD)                     // REQ #017
       { /* Wrong type */
...
BOOL SetRegistryDWordArray(HKEY root, const CString & var, const CDWordArray & data)// REG #006
    {
     ...
     result = ::RegSetValueEx(key, name, 0, REG_BINARY, (LPBYTE)data.GetData(),  // REG #006 
                              (DWORD)(UINT_PTR)data.GetSize() * (DWORD)(UINT_PTR)sizeof(DWORD));  // REG #019
     ::RegCloseKey(key);
     if(result != ERROR_SUCCESS)
        { /* failed */
         ::SetLastError(result);
...
BOOL SetRegistryInt64(HKEY root, const CString & var, __int64 val)
   {
    ...
    result = ::RegSetValueEx(key, name, 0, REG_QWORD, (LPBYTE)&val, sizeof(ULONGLONG)); // REG #017
    ::RegCloseKey(key);
