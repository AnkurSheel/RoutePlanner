...
static void setEvent(HWND hwnd, UINT CtlID, UINT event)
    {
     ::SetProp(hwnd, MAKEINTATOM(CTLID_BASE + CtlID), (HANDLE)(LONGLONG)(event + 1));  // REQ #006
    }

/****************************************************************************
*                                  getEvent
...
static UINT getEvent(HWND hwnd, UINT CtlID)
    {
     return (UINT)(((int)(LONGLONG)::GetProp(hwnd, 
                                       MAKEINTATOM(CTLID_BASE + CtlID))) - 1);  // REQ #006
    }

/****************************************************************************
*                                  setProc
...
void subclassCommand(CWnd * ctl, UINT id, UINT event)
    {
     ...
     WNDPROC oldproc = (WNDPROC)::GetWindowLongPtr(ctl->m_hWnd, GWLP_WNDPROC); // REQ #006
     if(oldproc != changeProc)
        setProc(ctl->m_hWnd, (WNDPROC)::SetWindowLongPtr(ctl->m_hWnd, GWLP_WNDPROC, (LONG_PTR)changeProc));  // REQ #006
    }

/****************************************************************************
*                              unsubclassControl
...
void unsubclassCommand(CWnd * ctl, UINT ctlID)
    {
     ...
     if(p != NULL)
        { /* restore */
         ::SetWindowLongPtr(ctl->m_hWnd, GWLP_WNDPROC, (LONG_PTR)p); // REQ #006
        } /* restore */
